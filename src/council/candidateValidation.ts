...
const SECTION_LABELS: Record<keyof CandidateValidationSignals, string> = {
  ...
  selfReviewAgainstOriginalTask: "Self-Review Against Original Task",
  hiddenProbeTestPlan: "Hidden Probe Test Plan",
  errorApiContractChecklist: "Public API / Error Contract Checklist",
  packageBuildChecklist: "package/build checklist",
};
...
export function buildCandidateRepairPrompt(input: { task: string; previousOutput: string }): string {
  ...
  return [
    ...
    "## 4. Public API / Error Contract Checklist",
    "## 5. Implementation Proposal (file tree, key code blocks, verification commands)",
    "## 6. Self-Review Against Original Task",
    "",
    "Original user task:",
    input.task,
    "",
